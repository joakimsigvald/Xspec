...
/// <summary>
/// Sequence constraints for From&lt;TSource&gt;() with a numeric source type,
/// controlling which values the generator produces for the registered type relay.
/// Generated values are guaranteed to be unique: fixed spacings wrap around at the type's
...
public static class UsingFromExtensions
{
    ...
    {
        Sequence(pipeline).SetStart(start, startExpr!);
        return pipeline;
    }
        ...
        where TSource : INumber<TSource>
    {
        ...
        Sequence(pipeline).SetStep((current, _) => current + spacing, spacingExpr!);
        return pipeline;
    }
        ...
        where TSource : INumber<TSource>
    {
        ...
        Sequence(pipeline).SetStep((current, _) => spacing(current), spacingExpr!);
        return pipeline;
    }
        ...
        where TSource : INumber<TSource>
    {
        if (spacing is null)
            throw new SetupFailed("Spacing cannot be null");
        Sequence(pipeline).SetStep(spacing, spacingExpr!);
        return pipeline;
    }

    private static NumericSequence<TSource> Sequence<TSUT, TResult, TSource>(
        IUsingFromContinuation<TSUT, TResult, TSource> pipeline)
        where TSource : INumber<TSource>
    {
        ...
        var sequence = receiver.Sequence as NumericSequence<TSource> ?? new();
